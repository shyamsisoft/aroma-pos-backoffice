...
const ReportsView: React.FC<ReportsViewProps> = ({ isDarkMode, permissions }) => {
    ...
    // Store current report data and columns for export/print
    const [currentReportData, setCurrentReportData] = useState<any[]>([]);
    const [currentReportColumns, setCurrentReportColumns] = useState<any[]>([]);
    ...
    // Helper to update current report state on render
    const updateCurrentReport = (data: any[], columns: any[], title: string) => {
        // We use a timeout to avoid set state during render loop, or just rely on the fact 
        // that we are calling the generators inside the render.
        // Better approach: Calculate data derived from state in the render function and assign it to a ref or let
        // But for export button onclick, we need access to it.
        // We will assign it to a Ref or let variable that persists for the handle functions.
        return { data, columns, title };
    };
    
    // We will use this object to store the currently rendered report configuration for the Print/Export buttons
    let reportConfig = { data: [] as any[], columns: [] as any[], title: '' };

    const handleMenuClick = (e: { key: string }) => {
        setMainCategory(e.key);
        // Set default sub-tabs based on category
    ...
    const handlePrint = () => {
        ...
        const tableRows = reportConfig.data.map(row => {
            return `<tr>${reportConfig.columns.map(c => {
                ...
                // basic formatting for currency if column key hints it (simple heuristic)
                                ...
                                items={currentData.tabs.map(tab => ({
                                    ...
                                    children: (
                                        <div style={{ padding: '0 24px 24px' }}>
                                            {/* Optional Summary Statistics Header for any report */}
