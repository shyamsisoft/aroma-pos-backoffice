...
import Employees from '../features/users/pages/Employees';
import Categories from '../features/menu/pages/Categories';
import Modifiers from '../features/menu/pages/Modifiers';
...
import { MenuItem, Category, ModifierGroup, Device, Employee, Activity, ModifierItem } from '../shared/types';
import { MOCK_MENU_ITEMS, MOCK_CATEGORIES, MOCK_MODIFIER_GROUPS, MOCK_DEVICES, MOCK_EMPLOYEES, MOCK_MODIFIER_ITEMS } from '../shared/constants';
...
const App: React.FC = () => {
  ...
  const [modifierItems, setModifierItems] = useState<ModifierItem[]>(MOCK_MODIFIER_ITEMS);
  ...
  const handleSaveModifierItem = (item: ModifierItem) => {
      if(modifierItems.find(i => i.id === item.id)) {
          setModifierItems(prev => prev.map(i => i.id === item.id ? item : i));
          logActivity('updated modifier item', item.name);
      } else {
          setModifierItems(prev => [...prev, item]);
          logActivity('created modifier item', item.name);
      }
      message.success("Modifier Item saved");
  };
  
  const handleDeleteModifierItem = (id: string) => {
      const item = modifierItems.find(i => i.id === id);
      // Also remove from any groups that reference it
      setModifierGroups(prev => prev.map(g => ({
          ...g,
          modifiers: g.modifiers.filter(m => m.modifierId !== id)
      })));
      
      setModifierItems(prev => prev.filter(i => i.id !== id));
      message.success("Modifier Item deleted");
      if(item) logActivity('deleted modifier item', item.name);
  };
          ...
          <Route path="/" element={currentUser ? <MasterLayout currentUser={currentUser} isDarkMode={isDarkMode} setIsDarkMode={setIsDarkMode} onLogout={handleLogout} /> : <Navigate to="/login" replace />}>
            ...
            <Route path="modifiers" element={
                <Modifiers 
                    ...
                    onSaveItem={handleSaveModifierItem}
                    onDeleteItem={handleDeleteModifierItem}
                />
            } />
